...
try:
    ...
    server.connect((host_client, port))
except Exception:
...
while True:
    ...
    print(f"\n{client_files_id:^26} command: {' '.join(server_auto_message)}")
    ...
    if 'ts' == auto_message[0] or 'tt' == auto_message[0]:
        ...
        print(f'\t (in_file): file_size: {file_size} is_file_found: {os.path.exists(infile_name)}')
        server.recv(buffer_size)
